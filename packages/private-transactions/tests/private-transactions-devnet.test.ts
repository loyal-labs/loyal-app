...
describeIfEnabled("private-transactions SDK (PER)", () => {
  ...
  const userKp = loadDevnetSmokeKeypair(
    "PRIVATE_TRANSACTIONS_DEVNET_USER_KEYPAIR"
  );
  ...
  it("runs deposit, delegate, private transfer, claim, and undelegate flow", async () => {
    ...
    let erUserDeposit = await ephemeralClientUser.getDeposit(user, tokenMint);
    for (let i = 0; i < 25 && !erUserDeposit; i += 1) {
      await sleep(200);
      erUserDeposit = await ephemeralClientUser.getDeposit(user, tokenMint);
    }
    ...
    try {
      ...
      try {
        ...
        console.error("[sdk-test] depositForUsername simulate logs", simLogs);
      } catch (simErr) {
