...
function useMainAccountUsdcBalance(args: {
  ...
  return {
    ...
    amountRaw,
    hasResolved: resolvedScope === balanceScope,
    refresh,
    setAmountRaw,
  };
}
...
export function AppWalletWorkspace({
  ...
  >({});
  const earnDepositLifecycleRef = useRef<LifecycleTracker | null>(null);
  const earnWithdrawLifecycleRef = useRef<LifecycleTracker | null>(null);
  const earnAutodepositLifecycleRef = useRef<LifecycleTracker | null>(null);
  // Close review must not adopt a terminal setup/pause/resume tracker.
  const earnAutodepositCloseLifecycleRef = useRef<LifecycleTracker | null>(
    null
  );
  ...
  const isEarnPositionInitialLoading =
    ...
    !hasEarnPosition;
  const pageLoadMetricSentRef = useRef(false);
  ...
  useEffect(() => {
    ...
    pageLoadMetricSentRef.current = true;
    captureBrowserLoadingMetricAfterPaint({
      operation: "page_load",
      phase: "balances_ready",
      startedAtMs: 0,
    });
  }, [areDisplayedBalancesReady]);
  ...
  const handleBackFromAutodeposit = useCallback(() => {
    ...
    const tracker =
      earnAutodepositCloseLifecycleRef.current ??
      earnAutodepositSetupAttemptRef.current?.tracker;
    tracker?.cancel("wallet_approval");
    if (earnAutodepositLifecycleRef.current === tracker) {
      earnAutodepositLifecycleRef.current = null;
    }
    earnAutodepositCloseLifecycleRef.current = null;
    earnAutodepositSetupAttemptRef.current = null;
    markDetailPaneTransition("back");
  ...
  const handleOpenAutodepositCloseReview = useCallback(() => {
    ...
    earnAutodepositLifecycleRef.current = tracker;
    earnAutodepositCloseLifecycleRef.current = tracker;
    tracker.start("intent");
    tracker.observe("prepare");
  ...
  const handleDismissEarnDepositPreview = useCallback(() => {
    earnDepositLifecycleRef.current?.cancel("review");
    setPendingEarnDepositDraft(null);
    setPendingEarnDepositPrepared(null);
    setEarnDepositReviewStage("deposit");
    setIsEarnDepositPolicySetupFlow(false);
  ...
  const handleDismissEarnWithdrawPreview = useCallback(() => {
    earnWithdrawLifecycleRef.current?.cancel("prepare");
    setPendingEarnWithdrawDraft(null);
    setPendingEarnWithdrawPrepared(null);
    setPendingEarnCleanupPrepared(null);
    setEarnWithdrawReviewStage("withdraw-0");
  ...
  const handleSubmitEarnDepositDraft = useCallback(
    async (draft: EarnDepositDraft) => {
      ...
      const tracker = createBrowserLifecycleTracker({
        ...
        flowVariant: hasEarnPosition
          ? "top_up"
          : smartAccountData.earnOnboarding
          ? "resumed"
          : "initial",
      });
      earnDepositLifecycleRef.current = tracker;
      const prepareStartedAtMs = getBrowserPerformanceNow();
      ...
      try {
        ...
        } else if (walletSubmittedAtMs !== null) {
        ...
        const failurePhase = resolveBrowserLoadingFailurePhase({
          previewMetricSent,
          walletSubmitted: walletSubmittedAtMs !== null,
        });
        if (failurePhase === "interaction_to_preview") {
          captureBrowserLoadingMetricAfterPaint({
            flowId: tracker.flowId,
            operation: "earn.deposit",
            outcome: "failed",
          ...
          failurePhase === "wallet_confirmation_to_ui" &&
          walletSubmittedAtMs !== null
        ) {
          captureBrowserLoadingMetricAfterPaint({
            flowId: tracker.flowId,
            operation: "earn.deposit",
  ...
  const handleSubmitEarnWithdrawDraft = useCallback(
    async (draft: EarnWithdrawDraft) => {
      ...
      earnWithdrawLifecycleRef.current = tracker;
      const prepareStartedAtMs = getBrowserPerformanceNow();
      let previewMetricSent = false;
      let walletSubmittedAtMs: number | null = null;
      tracker.start("intent", {
      ...
      try {
        ...
        const preparedWithdraw = await measureBrowserLoadingDependencies({
          flowId: tracker.flowId,
  ...
  const handleSubmitEarnCleanup = useCallback(async () => {
    const flowId = earnWithdrawLifecycleRef.current?.flowId;
    const prepareStartedAtMs = getBrowserPerformanceNow();
    setProposalActionError(null);
    setEarnDepositPrepareError(null);
    setPendingEarnWithdrawDraft(null);
    ...
    try {
      setIsEarnWithdrawPreparePending(true);
      earnWithdrawLifecycleRef.current?.observe("full_exit_verify", {
        chainState: "confirmed",
        cleanupRequired: true,
      });
      const prepareCleanup = () =>
        prepareEarnCleanupOnServer({ observabilityFlowId: flowId });
      const preparedCleanup = flowId
        ? await measureBrowserLoadingDependencies({
            flowId,
      ...
      setProposalActionError(message);
      setEarnDepositPrepareError(message);
      earnWithdrawLifecycleRef.current?.fail("full_exit_verify", {
  ...
  const handleContinueEarnDepositReview = useCallback(async () => {
    ...
    setEarnDepositPrepareError(null);
    setIsEarnAutoSigning(true);
    const tracker = earnDepositLifecycleRef.current;
    let walletSubmittedAtMs: number | null = null;
    try {
      ...
      if (stage === "policy" || stage === "policy-finalize") {
        ...
        if (!batchResult.batchUnavailable) {
          ...
          setDetailSelection("earn");
          setSelectedDetail("Earn");
          if (tracker && walletSubmittedAtMs !== null) {
            captureBrowserLoadingMetricAfterPaint({
              flowId: tracker.flowId,
              operation: "earn.deposit",
              phase: "wallet_confirmation_to_ui",
    ...
    try {
      ...
      for (;;) {
        ...
        tracker?.observe("wallet_submit_confirm", {
          chainState: "submitted",
          executionMode:
            preparedWithdraw.withdrawSteps.length > 1 ? "sequential" : "single",
        ...
        if (pendingEarnWithdrawDraft.mode === "full") {
  ...
  const handleCompleteEarnCleanup = useCallback(async () => {
    ...
    try {
      ...
      if (isWalletCancellation(error)) {
        ...
        tracker?.fail("cleanup", { errorCode: "unexpected_error" });
      }
      if (tracker) {
        captureBrowserSubmittedFailureMetricAfterPaint({
          flowId: tracker.flowId,
  ...
  const handleCompleteEarnAutodepositSetup = useCallback(async () => {
    ...
    const setupAttempt = earnAutodepositSetupAttemptRef.current;
    const existingTracker =
      setupAttempt &&
      earnAutodepositLifecycleRef.current === setupAttempt.tracker
        ? setupAttempt.tracker
        : null;
    const tracker =
      existingTracker ??
      createBrowserLifecycleTracker({
    ...
    try {
      ...
      if (earnAutodepositSetupAttemptRef.current?.tracker === tracker) {
        earnAutodepositSetupAttemptRef.current = null;
      }
      if (earnAutodepositLifecycleRef.current === tracker) {
        earnAutodepositLifecycleRef.current = null;
