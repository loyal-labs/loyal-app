...
- `docker build` succeeds. A local container runs the production startup smoke, accepts valid authenticated OTLP logs, metrics, and traces canaries, stores a uniquely named log in ClickHouse, and retains that log after recreating the container against the same volume.
