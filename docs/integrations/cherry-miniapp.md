...
| Auth                        | `apps/web/src/features/cherry/client/privy-sign-in.tsx`; `apps/web/src/features/identity/server/privy-auth.ts`                                                                                                      | Privy headless SIWS: the verified Cherry wallet signs Privy's message, passes the signature to Privy as base64, then the identity token is exchanged at `/api/auth/privy/complete`. No Privy modal, no captcha. Legacy wallet-proof path only when Privy is off. |
