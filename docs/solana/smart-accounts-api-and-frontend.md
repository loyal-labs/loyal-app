...
read, withdrawal source listing, autodeposit state, the fixed `30D` earnings
series used by the native chart, and the newest-first Earn transaction history
shown in the native Activity > Earn tab.
...
| Passive reads            | `GET /api/smart-accounts/mobile/earn/state`, `GET /api/smart-accounts/mobile/earn/earnings`, `GET /api/smart-accounts/mobile/earn/transactions`                                                                                         | `state` mirrors the session Earn position/state reads, `earnings` always returns the native chart's `30D` range, and `transactions` is the wallet-addressed twin of the session Earn history feed. |
...
`frontend/src/app/api/smart-accounts/mobile/earn/earnings/route.ts`, and
`frontend/src/app/api/smart-accounts/mobile/earn/transactions/route.ts`.
