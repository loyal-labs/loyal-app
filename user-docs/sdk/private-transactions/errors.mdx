...
<img
  alt="Private Transactions SDK error-handling overview"
  className="block dark:hidden"
  src="/resources/private-txn/errors-light.png"
/>

<img
  alt="Private Transactions SDK error-handling overview"
  className="hidden dark:block"
  src="/resources/private-txn/errors-dark.png"
/>
...
- `"Account is delegated to ER: ..."` means a base-layer operation (`modifyBalance`, `createPermission`, `initializeDeposit`) was attempted on a delegated account. Undelegate first.
- `"Account is not delegated to ER: ..."` means a PER operation (`transferDeposit`, `transferToUsernameDeposit`, `claimUsernameDepositToDeposit`) was attempted before delegation. Delegate first.
- `"Account is delegated on wrong validator: ..."` means the delegation authority differs from the expected `ER_VALIDATOR`.
...
| `NotVerified` | `"Not Verified"` | Telegram session is not verified. Complete `store` and `verify_telegram_init_data` first. |
...
| `InvalidUsername` | `"Invalid Username"` | Username fails validation, or the session username differs from the username deposit. |
| `InvalidRecipient` | `"Invalid Recipient"` | Recipient wallet differs from the expected claim target. |
...
3. Token mint alignment. Confirm every related account uses the same token mint.
...
5. Insufficient balance. `InsufficientDeposit` means the deposit's `amount` is less than the requested transfer or withdrawal. Check with `getBaseDeposit()` or `getEphemeralDeposit()`.

6. Username validation. Usernames must be 5 to 32 characters from `[a-zA-Z0-9_]`. The SDK and program both validate this rule.
