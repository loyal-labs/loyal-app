...
| Surface | Exports |
| --- | --- |
| Features | `programConfig`, `smartAccounts`, `proposals`, `transactions`, `batches`, `policies`, `spendingLimits`, `execution` |
| Protocol helpers | `generated`, `accounts`, `pda`, `codecs`, `errors`, `PROGRAM_ID` |
...
Each feature namespace can expose account classes, raw builders, prepared operations, queries, or bound client methods. Availability depends on the feature.

| Namespace | Representative operations |
| --- | --- |
| Proposals | `create`, `approve`, `reject`, `cancel` |
| Transactions | `create`, `buffer`, `logEvent` |
| Execution | Async or synchronous paths for settings, batches, or policies |
...
Most Loyal frontend flows use `@loyal-labs/smart-account-vaults` on top of this SDK. Read [Frontend and Vault APIs](/smart-accounts/frontend-and-vaults) for its read model, prepared writes, agent connection, and diagnostics.
