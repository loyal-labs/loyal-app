...
- `SOLANA_WEEK_QUESTS_ENABLED` - Master kill-switch; reporting only goes live when this is exactly `"true"`, unless a wallet is allowlisted below
- `SOLANA_WEEK_QUESTS_TESTER_WALLETS` - Optional comma-separated base58 tester wallet allowlist that bypasses the kill-switch while quests stay hidden
- `SOLANA_WEEK_QUESTS_COMPLETION_ENDPOINT` - Challenge Quest Completion API endpoint; the reporter no-ops until this, the API key, and the relevant quest id are set
...
- `SOLANA_WEEK_QUEST_ID_EARN_DEPOSIT` - Quest 1 id for the manual Earn deposit completion
