...
const UPSTREAM_IDL_PATH =
  "sdk/smart-account/idl/squads_smart_account_program.json";
...
export function validateIdlSurface(idl: SmartAccountIdl): void {
  ...
  const accountNames = new Set(
    (idl.accounts ?? []).map((account) => account.name)
  );
...
function assertFeatureCoverage(): void {
  ...
  if (
    ...
    issues.duplicateExports.length === 0
  ) {
    return;
  }
...
function assertRustBindingsCompile(): void {
  execFileSync("cargo", ["check", "-p", "loyal-smart-accounts-rs"], {
    cwd: ROOT_DIR,
    stdio: "inherit",
  });
}
...
function renderRustGeneratedConstants(
  programId: string,
...
function resolveUpstreamDirectory(options: SyncOptions): string {
  ...
  if (options.ref) {
    execFileSync(
      "git",
      ["-C", cloneDir, "fetch", "--depth=1", "origin", options.ref],
      {
        stdio: "inherit",
      }
    );
...
export async function syncFromUpstream(options: SyncOptions): Promise<{
  ...
  const upstreamPackageJsonPath = join(
    upstreamDir,
    "sdk/smart-account/package.json"
  );
  ...
  const libProgramId = parseProgramIdFromLibRs(
    readFileSync(programLibPath, "utf8")
  );
  ...
  assertOrWriteFile({
    ...
    content: JSON.stringify(
      await buildCreateSmartAccountParityFixture(),
      null,
      2
    ),
    check: options.check,
  ...
  writeFileSync(
    join(tempRustGeneratedDir, "mod.rs"),
    renderRustGeneratedMod(),
    "utf8"
  );
  writeFileSync(
...
async function main(): Promise<void> {
  ...
  console.log(
    `${options.check ? "Validated" : "Updated"} loyal-smart-accounts from ${
      options.repoUrl
    } at ${result.commitSha} (${result.programId})`
  );
}
