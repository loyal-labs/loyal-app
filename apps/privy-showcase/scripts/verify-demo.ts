...
async function verifyStatic() {
  ...
  const relativeRoutes = apiRoutes.map((path) => relative(root, path)).sort();
  record("surface.one_backend_route", relativeRoutes.length === 2 && relativeRoutes[0] === "src/app/api/rpc/route.ts" && relativeRoutes[1] === "src/app/api/sponsor/route.ts", "one authenticated backend route owns setup and fixed money moves, plus the same-origin RPC passthrough", `found API routes: ${relativeRoutes.join(", ")}`);

  const rpcProxy = executableSource.includes("enforceSameOrigin(request)") && executableSource.includes('new URL("/api/rpc", window.location.origin)') && executableSource.includes("DEMO_SERVER_RPC_URL");
  record("surface.rpc_proxy_same_origin", rpcProxy, "browser JSON-RPC rides the same-origin proxy; the server key never reaches the browser", "the RPC passthrough is missing its same-origin guard or the browser still calls the upstream directly");
  ...
  const requiredActions = ["Continue with email", "Withdraw USDC", "Set up the account", "Run the loop", "Reset the demo", "Finish the reset", "Move 2 USDC to smart account", "Move 2 USDC to Kamino", "Move 1 USDC back to smart account", "Send 1 USDC to wallet"];
  const primaryActions = page.match(/data-primary-action/g) ?? [];
  record("ui.separate_steps_and_moves", primaryActions.length === requiredActions.length && requiredActions.every((label) => page.includes(label)), "connect, wallet withdrawal, the progressive setup, the full loop, and each fixed move are separate explained actions", `found ${primaryActions.length} primary markers; required labels=${requiredActions.join(" | ")}`);

  const loopChaining = page.includes("LOOP_STEPS") && page.includes('"wallet_to_smart_account"') && page.includes('"smart_account_to_kamino"') && page.includes('"kamino_to_smart_account"') && page.includes('"smart_account_to_wallet"') && page.includes("runLoop") && page.includes("createAccountInner") && page.includes("createPoliciesInner") && page.includes("runSetup");
  record("ui.scenario_chaining", loopChaining, "the loop chains exactly the four fixed backend moves and setup chains account creation then policies, nothing else", "the loop or the progressive setup does not decompose into the fixed backend actions");

  const fullReset = page.includes("teardownDemo") && executableSource.includes("resolveNextTeardownStage") && executableSource.includes("prepareTeardownStage") && executableSource.includes("prepareEarnUsdcWithdraw") && executableSource.includes("prepareEarnVaultAccountsRefund") && executableSource.includes("prepareEarnUsdcAutodepositClose") && executableSource.includes("prepareClosePoliciesSync") && executableSource.includes("rentCollector") && executableSource.includes('"teardown-withdraw"') && executableSource.includes('"teardown-exit"');
  record("teardown.full_reset", fullReset, "the reset drains Kamino, closes all four policies through wallet-signed stages, and the sponsor re-derives each stage", "the demo reset does not decompose into the wallet-signed teardown stages");
